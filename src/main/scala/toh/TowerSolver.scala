...
object Peg{
  def otherPeg(first: Peg, second: Peg) : Peg = Vector(Left, Right, Middle).filter ((x) => x != first && x != second).head
}
...
object TowerSolver{
  def solve(start: TowerState, goal: TowerState): Vector[Move] = {
    ...
    if (rings == 0) Vector()
      ...
      if (source == sink)
        solve(start.init, goal.init)
      else {
        val third = Peg.otherPeg(source, sink)
        val onThird = TowerState.simple(rings - 1, third)
        (solve(start.init, onThird) :+ Move(source, sink)) ++ solve(onThird, TowerState.simple(rings - 1, sink))
      }
    }
  }

  def apply(start: Peg, goal: Peg, rings: Int) = solve(TowerState.simple(rings, start), TowerState.simple(rings, goal))
}
...
object TowerState{
  def simple(rings: Int, peg: Peg) = TowerState(rings, ((1 to rings) map (_ -> peg)).toMap)
}
